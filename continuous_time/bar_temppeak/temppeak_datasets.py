...
alpha = 0.0075 # see source for this problem
...
for i, x_i in enumerate(x):
    for j, t_j in enumerate(t):
        ...
        for n in range(1,num_series_terms+1):
            u_tjxi += 400/(math.pi * n) * math.sin(math.pi*n/2) * math.sin(0.05*math.pi*n) * math.sin(n*math.pi*x_i) * math.exp(-alpha*(n**2)*(math.pi**2)*t_j)
        # store result
...
upper_limits = np.array([5, 1])  # upper limits for both variables (respectively)
...
for i in range(N_boundary):
    ...
    if boundary == 0: # initial points
        ...
        if x > 0.45 and x < 0.55: # if the point is in the hot portion
            u = 100
        else:  # if the point is in the cold portion
            u = 0
